function getAtagawa() {
  ...
  // spread sheetのシート設定
  var sheet = spreadsheet.getSheetByName(spot_data.Atagawa.sheet_name);
  
  // ダイビングスポット別のAPI-URLの作成
  var fb_api_url = fb_url + spot_data.Atagawa.fb_path + "/feed?access_token=" + access_token + "&limit=" + limit;
  ...
  for (var i = 0; i < api_loop; i++) {
    ...
    for (var j = 0; j < jsonData.data.length; j++) {            
      ...
      var arr_tmp = [
        ...
        (getNum(msg, "水温", "度", "avg") != null) ? getNum(msg, "水温", "度", "avg") : getNum(msg, "水温", "℃", "avg"),
        (getNum(msg, "透明度", "m", "min") != null) ? getNum(msg, "透明度", "m", "min") : getNum(msg, "透明度", "ｍ", "min"),
        msg
      ];
