function getIzuohshima() {
  // スクリプトの開始時刻 (スクリプトの実行時間測定用)
  var start = new Date();
  
  // spread sheetのシート設定
  var sheet = spreadsheet.getSheetByName(spot_data.Izuohshima.sheet_name);
  
  // ダイビングスポット別のAPI-URLの作成
  var fb_api_url = fb_url + spot_data.Izuohshima.fb_path + "/feed?access_token=" + access_token + "&limit=" + limit;
  ...
  for (var i = 0; i < api_loop; i++) {
    ...
    for (var j = 0; j < jsonData.data.length; j++) {            
      ...
      var arr_tmp = [
        ...
        (getNum(msg, "水温", "℃", "avg") != null) ? getNum(msg, "水温", "℃", "avg") : getNum(msg, "水温", "℃", "avg"),
        (getNum(msg, "透明度", "ｍ", "min") != null) ? getNum(msg, "透明度", "ｍ", "min") : getNum(msg, "透明度", "ｍ", "min"),
        msg
      ];
      
      // 水温と透明度が異常値の場合スキップ
      if (arr_tmp[1] > 40 || arr_tmp[2] > 40 ) {
//        continue;
      }
