function getJogashima() {
  // スクリプトの開始時刻 (スクリプトの実行時間測定用)
  var start = new Date();
  
  // spread sheetのシート設定
  var sheet = spreadsheet.getSheetByName(spot_data.Jogashima.sheet_name);
  
  // ダイビングスポット別のAPI-URLの作成
  var fb_api_url = fb_url + spot_data.Jogashima.fb_path + "/feed?access_token=" + access_token + "&limit=" + limit;
  ...
  for (var i = 0; i < api_loop; i++) {
    ...
    for (var j = 0; j < jsonData.data.length; j++) {            
      ...
      var horizon;      
      if (getNum(msg, "の透明度", "ｍ", "min") != null) {
        horizon = getNum(msg, "の透明度", "ｍ", "min");
      }
      if (getNum(msg, "●透明度：", "ｍ", "min") != null) {
        horizon = getNum(msg, "●透明度：", "ｍ", "min");
      }
