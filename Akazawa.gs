function getAkazawa() {
  ...
  // spread sheetのシート設定
  var sheet = spreadsheet.getSheetByName(spot_data.Akazawa.sheet_name);
  
  // ダイビングスポット別のAPI-URLの作成
  var fb_api_url = fb_url + spot_data.Akazawa.fb_path + "/feed?access_token=" + access_token + "&limit=" + limit;
  ...
  for (var i = 0; i < api_loop; i++) {
    ...
    for (var j = 0; j < jsonData.data.length; j++) {            
      ...
      var arr_tmp = [
        ...
        getNum(msg, "水温", "度", "avg"),
        (getNum(msg, "透視", "水温", "min") != null) ? getNum(msg, "透視", "水温", "min") : getNum(msg, "透明度", "水温", "min"),
        msg
      ];
