function getFuto() {
  ...
  // spread sheetのシート設定
  var sheet = spreadsheet.getSheetByName(spot_data.Futo.sheet_name);
  
  // ダイビングスポット別のAPI-URLの作成
  var fb_api_url = fb_url + spot_data.Futo.fb_path + "/feed?access_token=" + access_token + "&limit=" + limit;
  ...
  for (var i = 0; i < api_loop; i++) {
    ...
    for (var j = 0; j < jsonData.data.length; j++) {            
      ...
      var arr_tmp = [
        ...
        getNum(msg, "水温", "℃", "avg"),
        (getNum(msg, "透明度", "ｍ", "min") != null) ? getNum(msg, "透明度", "ｍ", "min") : getNum(msg, "透明度", "m", "min"),
        msg
      ];
